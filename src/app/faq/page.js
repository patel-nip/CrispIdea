...
export default function FAQ() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#1F2922] font-sans selection:bg-[#4A6B52]/20 selection:text-[#1F2922]">
      ...
      {/* HERO SECTION */}
      <section className="relative overflow-hidden bg-grid pt-16 pb-16 px-6 lg:px-12 border-b border-[#4A6B52]/10">
        <div className="max-w-4xl mx-auto text-center">
          <span className="bg-white/80 backdrop-blur-sm border border-[#4A6B52]/15 rounded-full px-3 py-1 text-xs font-semibold inline-flex items-center gap-1.5 mb-6 shadow-sm">
            <HelpCircle className="w-3.5 h-3.5 text-[#4A6B52]" />
            FAQ Help Center
          </span>
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-[#1F2922] leading-[1.15] mb-6">
            Frequently Asked Questions
          </h1>
          <p className="text-sm md:text-base text-[#1F2922]/70 leading-relaxed max-w-xl mx-auto mb-8 font-medium">
            Everything you need to know about our fiduciary wealth advisory engine and how we structure your portfolios.
          </p>

          {/* Search bar */}
          <div className="max-w-md mx-auto relative">
            <Search className="w-4 h-4 text-[#1F2922]/40 absolute left-4 top-1/2 -translate-y-1/2" />
            <input 
              type="text" 
              placeholder="Search questions or keywords..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-11 pr-4 py-3 bg-white border border-[#4A6B52]/15 rounded-full text-sm placeholder-[#1F2922]/45 focus:outline-none focus:border-[#4A6B52]/35 focus:ring-1 focus:ring-[#4A6B52]/35 shadow-sm transition"
            />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 lg:px-12 bg-white flex-1">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-2 mb-8 border-b border-[#4A6B52]/10 pb-4">
            <TrendingUp className="w-5 h-5 text-[#4A6B52]" />
            <h2 className="text-base font-bold text-[#1F2922] uppercase tracking-widest font-sans">
          ...
          {filteredFaqs.length > 0 ? (
            <div className="space-y-4">
              {filteredFaqs.map((faq, idx) => {
                ...
                return (
                  <div 
                    key={globalIdx}
                    className="border border-[#4A6B52]/10 rounded-2xl bg-[#FAF8F5]/30 overflow-hidden transition-all duration-300"
                  >
                    <button
                      onClick={() => toggleFaq(globalIdx)}
                      className="w-full px-6 py-5 flex items-center justify-between text-left font-bold text-sm md:text-base text-[#1F2922] focus:outline-none hover:bg-[#FAF8F5]/60 transition"
                    >
                      <span className="pr-4">{faq.question}</span>
                      <ChevronDown className={`w-4 h-4 text-[#4A6B52]/70 shrink-0 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} />
                    </button>
                    <div 
                      className={`grid transition-all duration-300 ease-in-out ${isOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
                    >
                      <div className="overflow-hidden">
                        <div className="px-6 pb-6 pt-1 text-xs md:text-sm text-[#1F2922]/70 leading-relaxed font-medium">
                          {faq.answer}
                        </div>
                      </div>
                    </div>
            ...
            <div className="text-center py-12 text-[#1F2922]/60">
              <p className="text-sm font-semibold">No questions found matching "{searchQuery}"</p>
              <button 
                onClick={() => setSearchQuery("")}
                className="text-xs text-[#4A6B52] underline font-bold mt-2 hover:text-black"
              >
                Clear search filter
              </button>
            </div>
          ...
          <div className="mt-16 bg-gradient-to-br from-[#F0F5FA] to-white/95 border border-blue-500/10 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm">
            <div className="flex gap-4 items-start">
              <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 shrink-0">
                <Shield className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-sm font-bold text-[#1F2922] mb-1">Fiduciary & Filer Standards</h4>
                <p className="text-xs text-[#1F2922]/65 leading-relaxed max-w-md">
            ...
            <a 
              href="/#contact"
              className="inline-flex items-center gap-1 text-xs font-bold text-white bg-[#071E3D] hover:bg-[#030E1C] px-5 py-3 rounded-full shadow-sm transition shrink-0 group"
            >
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-6 gap-8 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-2.5 font-semibold">
              ...
              <li><a href="/faq" className="text-[#4A6B52] hover:text-[#4A6B52] transition">FAQ</a></li>
            </ul>
          </div>
